...
│   ├── agents/          # Momentum, Reversal, Liquidation
...
## Agents
1. **MOMENTUM** - Suit la tendance forte
2. **REVERSAL** - Contre-tendance sur exces
3. **LIQUIDATION** - Chasse les cascades de liquidations
...
- **Momentum**: Analyse momentum 1min et 5min pour determiner la tendance (HAUSSIER/BAISSIER/NEUTRE)
...
- **TP/SL**: Take Profit et Stop Loss bases sur le spread (ex: TP = spread x 4, SL = spread x 2)
...
- Priorité actuelle : S'assurer que G10 fonctionne correctement (ouverture/fermeture de positions, tracking P&L, logging, API,etc.) Ensuite : Optimiser/créer une stratégie rentable.
- Priorité actuelle : S'assurer que G10 fonctionne correctement (ouverture/fermeture de positions, tracking P&L, logging, API,etc.) Ensuite : Optimiser/créer une stratégie rentable.
## Code
...
- **Agent config** (timeframe, intervalle, position size, TP/SL, cooldown): `onchange="saveAgentConfig('momentum|reversal|liquidation')"`
- **MT5 accounts** (login, server, password, enabled): `onchange="saveAgentAccount('momentum|reversal|liquidation')"`
