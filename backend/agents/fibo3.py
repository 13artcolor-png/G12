...
class Fibo3Agent(BaseAgent):
    ...
    def _calculate_fibo_levels(self, swing_high: float, swing_low: float,
        ...
        levels = {}
        for name, ratio in self.FIBO3_LEVELS.items():
            if is_uptrend:
    ...
    def _get_institutional_analysis(self, candles: List[Dict]) -> Optional[Dict]:
        ...
        try:
            ...
            print(f"[FIBO3] Erreur analyse institutionnelle: {e}")
            return None
...
=== TA MISSION ===
Tu es l'agent FIBO3. Tu trades sur les niveaux de Fibonacci avec confirmation ICT/SMC.

LOGIQUE FIBO3NACCI + ICT:
...
def get_fibo3_agent() -> Fibo3Agent:
    """Factory pour l'agent fibo3"""
    return Fibo3Agent()
