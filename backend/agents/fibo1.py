...
class Fibo1Agent(BaseAgent):
    ...
    def _calculate_fibo_levels(self, swing_high: float, swing_low: float,
        ...
        levels = {}
        for name, ratio in self.FIBO1_LEVELS.items():
            if is_uptrend:
    ...
    def _get_institutional_analysis(self, candles: List[Dict]) -> Optional[Dict]:
        ...
        try:
            ...
            print(f"[FIBO1] Erreur analyse institutionnelle: {e}")
            return None
...
=== TA MISSION ===
Tu es l'agent FIBO1. Tu trades sur les niveaux de Fibonacci avec confirmation ICT/SMC.

LOGIQUE FIBO1NACCI + ICT:
...
def get_fibo1_agent() -> Fibo1Agent:
    """Factory pour l'agent fibo1"""
    return Fibo1Agent()
